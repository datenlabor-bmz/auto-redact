...
export const uploadPdf = async (
  ...
  onFileUpload: (fileUrl: string, file: File, highlights: Array<SecuredactHighlight>) => void,
) => {
  ...
  if (file) {
    try {
      ...
      const fileWithoutRedactionAnnotations = new File([pdfBlob], file.name, { type: 'application/pdf' });
